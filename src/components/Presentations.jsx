...
const Presentations = () => {
  // All presentation files are stored in: public/documents2/
  const presentationData = [
    {
      title: "Progress Presentation 1",
      icon: "🖥️",
      fileName: "progress-presentation-1.pdf",
      filePath: `${process.env.PUBLIC_URL}/documents2/IT21291500 _ Premajayantha W H S I.pdf`
    },
    {
      title: "Progress Presentation 2",
      icon: "🖥️",
      fileName: "progress-presentation-2.pdf",
      filePath: `${process.env.PUBLIC_URL}/documents2/IT21291500 _ Premajayantha W H S I_Edited.pdf`
    },
    {
      title: "Final Presentation",
      icon: "🖥️",
      fileName: "final-presentation.pdf",
      filePath: `${process.env.PUBLIC_URL}/documents2/IT21291500 _ Premajayantha W H S I_Final.pdf`
    }
  ];

  // Handles file download for public assets
  const handleDownload = (filePath, fileName) => {
    const link = document.createElement('a');
    link.href = filePath;
    link.download = fileName;
  ...
  // Optional: For fetching files from an API or backend server
  const handleDownloadFromServer = async (filePath, fileName) => {
    try {
      const response = await fetch(filePath);
      if (!response.ok) throw new Error('Download failed');
    ...
    <section id="presentations">
      <div className="container-presentations">
        ...
        <p className="section-subtitle">
          View our project presentations throughout the development lifecycle.
        </p>
        ...
        <div className="documents-list">
          {presentationData.map((presentation, index) => (
            <DocumentItem
              ...
              icon={presentation.icon}
              onDownload={() =>
                handleDownload(presentation.filePath, presentation.fileName)
              }
              // Alternative if fetching from server:
              // onDownload={() => handleDownloadFromServer(presentation.filePath, presentation.fileName)}
            />
          ))}
        </div>
      </div>
